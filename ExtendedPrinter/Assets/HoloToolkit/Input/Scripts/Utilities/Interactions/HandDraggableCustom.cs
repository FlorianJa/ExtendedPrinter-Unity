...
namespace HoloToolkit.Unity.InputModule
{
    ...
    /// </summary>
    public class HandDraggableCustom : MonoBehaviour, IFocusable, IInputHandler, ISourceStateHandler
    {
        /// <summary>
        ...
        private void UpdateDragging()
        {
            ...
            Vector3 newPosition = HostTransform.localPosition;
            var objRefGrabPointInWorld = cameraTransform.TransformDirection(objRefGrabPoint);
            var targetPostion = draggingPosition + objRefGrabPointInWorld;
            var lerp = Vector3.Lerp(HostTransform.position, targetPostion, PositionLerpSpeed);
            var lerpInLocalSpace = HostTransform.parent.InverseTransformPoint(lerp);

            if(MoveX)
            {
                newPosition.x = Mathf.Clamp(lerpInLocalSpace.x, XMin, XMax);
            }
            if (MoveY)
            {
                newPosition.y = Mathf.Clamp(lerpInLocalSpace.y, YMin, YMax);
            }
            if (MoveZ)
            {
                newPosition.z = Mathf.Clamp(lerpInLocalSpace.z, ZMin, ZMax);
            }

            // Apply Final Position
            if (hostRigidbody == null)
            {
                HostTransform.localPosition = newPosition;
            }
            else
            {
